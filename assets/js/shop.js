...
function updateCart() {
    ...
    cart.forEach(item => {
        cartItemsHTML += `
        <li>
            <div>${item.productName} - LKR ${item.price}</div>
            <div class="quantity">
...
function getUserDetails() {
    ...
    if (!name.trim() || !email.trim() || !address.trim() || !phone.trim()) {
        alert("Please enter your name, email, address and phone number before checking out.");
    } else{
...
function confirmPayment() {
    ...
    if (!cardNumber.trim() || !expiryDate.trim() || !cvv.trim()) {
        ...
        alert(`Payment confirmed! Thank you! An email has been sent to your email.`);
